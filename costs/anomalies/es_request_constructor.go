...
func createAnomalyQueryTimeRange(durationBegin time.Time, durationEnd time.Time) *elastic.RangeQuery {
...
// getProductAnomaliesElasticSearchParams is used to construct an ElasticSearch *elastic.SearchService
...
func getProductAnomaliesElasticSearchParams(accountList []string, durationBegin time.Time,
	...
	query = query.Filter(createAnomalyQueryTimeRange(durationBegin, durationEnd))
	search := client.Search().Index(index).Size(queryMaxSize).Sort("date", false).Query(query)
	return search
}
...
func getElasticSearchParams(accountList []string, durationBegin time.Time,
	...
	search.Aggregation("products", elastic.NewTermsAggregation().Field("productCode").Size(aggregationMaxSize).
		SubAggregation("dates", elastic.NewDateHistogramAggregation().Field("usageStartDate").ExtendedBounds(durationBegin, durationEnd).Interval("day").
