...
// createQueryAccountFilterEbs creates and return a new *elastic.TermsQuery on the accountList array
func createQueryAccountFilterEbs(accountList []string) *elastic.TermsQuery {
...
func getElasticSearchEbsDailyParams(params EbsQueryParams, client *elastic.Client, index string) *elastic.SearchService {
	query := elastic.NewBoolQuery()
	if len(params.AccountList) > 0 {
		query = query.Filter(createQueryAccountFilterEbs(params.AccountList))
	}
...
//	- If the client is nil or malconfigured, it will crash
//	- If the index is not an index present in the ES, it will crash
func getElasticSearchEbsMonthlyParams(params EbsQueryParams, client *elastic.Client, index string) *elastic.SearchService {
	query := elastic.NewBoolQuery()
	if len(params.AccountList) > 0 {
		query = query.Filter(createQueryAccountFilterEbs(params.AccountList))
	}
...
//	- If the client is nil or malconfigured, it will crash
//	- If the index is not an index present in the ES, it will crash
func getElasticSearchCostParams(params EbsQueryParams, client *elastic.Client, index string) *elastic.SearchService {
	query := elastic.NewBoolQuery()
	if len(params.AccountList) > 0 {
		query = query.Filter(createQueryAccountFilterBill(params.AccountList))
	}
	query = query.Filter(elastic.NewTermsQuery("productCode", "AmazonEBS", "AmazonCloudWatch"))
	dateStart, dateEnd := getDateForDailyReport(params.Date)
