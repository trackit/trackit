...
// createQueryAccountFilterElastiCache creates and return a new *elastic.TermsQuery on the accountList array
func createQueryAccountFilterElastiCache(accountList []string) *elastic.TermsQuery {
...
// getElasticSearchElastiCacheDailyParams is used to construct an ElasticSearch *elastic.SearchService used to perform a request on ES
// It takes as parameters :
// 	- params ElastiCacheQueryParams : contains the list of accounts and the date
...
func getElasticSearchElastiCacheDailyParams(params ElastiCacheQueryParams, client *elastic.Client, index string) *elastic.SearchService {
	query := elastic.NewBoolQuery()
	if len(params.AccountList) > 0 {
		query = query.Filter(createQueryAccountFilterElastiCache(params.AccountList))
	}
...
// getElasticSearchElastiCacheMonthlyParams is used to construct an ElasticSearch *elastic.SearchService used to perform a request on ES
// It takes as parameters :
// 	- params ElastiCacheQueryParams : contains the list of accounts and the date
//	- client *elastic.Client : an instance of *elastic.Client that represent an Elastic Search client.
//	It needs to be fully configured and ready to execute a client.Search()
...
//	- If the client is nil or malconfigured, it will crash
//	- If the index is not an index present in the ES, it will crash
func getElasticSearchElastiCacheMonthlyParams(params ElastiCacheQueryParams, client *elastic.Client, index string) *elastic.SearchService {
	query := elastic.NewBoolQuery()
	if len(params.AccountList) > 0 {
		query = query.Filter(createQueryAccountFilterElastiCache(params.AccountList))
	}
...
//	- If the client is nil or malconfigured, it will crash
//	- If the index is not an index present in the ES, it will crash
func getElasticSearchCostParams(params ElastiCacheQueryParams, client *elastic.Client, index string) *elastic.SearchService {
	query := elastic.NewBoolQuery()
	if len(params.AccountList) > 0 {
		query = query.Filter(createQueryAccountFilterBill(params.AccountList))
	}
	query = query.Filter(elastic.NewTermQuery("productCode", "AmazonElastiCache"))
	dateStart, dateEnd := getDateForDailyReport(params.Date)
