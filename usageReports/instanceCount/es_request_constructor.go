...
// getElasticSearchInstanceCountDailyParams is used to construct an ElasticSearch *elastic.SearchService used to perform a request on ES
...
func getElasticSearchInstanceCountDailyParams(params InstanceCountQueryParams, client *elastic.Client, index string) *elastic.SearchService {
...
// It takes as parameters :
// 	- params InstanceCountQueryParams : contains the list of accounts and the date
//	- client *elastic.Client : an instanceCount of *elastic.Client that represent an Elastic Search client.
//	It needs to be fully configured and ready to execute a client.Search()
...
//	- If the client is nil or malconfigured, it will crash
//	- If the index is not an index present in the ES, it will crash
func getElasticSearchCostParams(params InstanceCountQueryParams, client *elastic.Client, index string) *elastic.SearchService {
	query := elastic.NewBoolQuery()
	if len(params.AccountList) > 0 {
		query = query.Filter(createQueryAccountFilterBill(params.AccountList))
	}
	query = query.Filter(elastic.NewTermsQuery("productCode", "AmazonInstanceCount", "AmazonCloudWatch"))
	dateStart, dateEnd := getDateForDailyReport(params.Date)
	...
	search.Aggregation("accounts", elastic.NewTermsAggregation().Field("usageAccountId").Size(maxAggregationSize).
		SubAggregation("reports", elastic.NewTermsAggregation().Field("resourceId").Size(maxAggregationSize).
