...
import (
	...
	indexSource "github.com/trackit/trackit/aws/usageReports/s3"
...
func processHit(ctx context.Context, hit *elastic.SearchHit, resourceTypeString string) (taggingReports.TaggingReportDocument, bool) {
	logger := jsonlog.LoggerFromContextOrDefault(ctx)
	var source indexSource.BucketReport
	...
	regionForURL := utils.GetRegionForURL(source.Bucket.Region)

	document := taggingReports.TaggingReportDocument{
		Account:      source.Account,
		ResourceID:   source.Bucket.Name,
		ResourceType: resourceTypeString,
		Region:       source.Bucket.Region,
		URL:          fmt.Sprintf(urlFormat, source.Bucket.Name, regionForURL),
		Tags:         source.Bucket.Tags,
	}
