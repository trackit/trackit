...
func fetchReports(ctx context.Context, awsAccount aws.AwsAccount) ([]*elastic.SearchHit, error) {
	client := es.Client
	indexName := es.IndexNameForUserId(awsAccount.UserId, indexSource.IndexPrefixESReport)
...
func queryEs(ctx context.Context, indexName string, awsAccount aws.AwsAccount) (*elastic.SearchResult, error) {
	...
	topHitsAggregation := elastic.NewTopHitsAggregation().Size(2147483647).FetchSourceContext(elastic.NewFetchSourceContext(true).Include("account", "domain.domainId", "domain.region", "domain.tags"))
