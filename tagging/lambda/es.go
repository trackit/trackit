...
import (
	...
	indexSource "github.com/trackit/trackit/aws/usageReports/lambda"
	"github.com/trackit/trackit/es"
)

func fetchReports(ctx context.Context, awsAccount aws.AwsAccount) ([]*elastic.SearchHit, error) {
	client := es.Client
	indexName := es.IndexNameForUserId(awsAccount.UserId, indexSource.IndexPrefixLambdaReport)
...
func queryEs(ctx context.Context, indexName string, awsAccount aws.AwsAccount) (*elastic.SearchResult, error) {
	...
	topHitsAggregation := elastic.NewTopHitsAggregation().Size(2147483647).FetchSourceContext(elastic.NewFetchSourceContext(true).Include("account", "function.name", "function.region", "function.tags"))
