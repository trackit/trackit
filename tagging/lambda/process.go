...
// Package tagginglambda implements the generation of tagging reports from Lambda reports
...
const urlFormat = "https://console.aws.amazon.com/lambda/home?region=%s#/functions/%s"
...
func processHit(ctx context.Context, hit *elastic.SearchHit, resourceTypeString string) (utils.TaggingReportDocument, bool) {
	logger := jsonlog.LoggerFromContextOrDefault(ctx)
	var source indexSource.FunctionReport
	...
	document := utils.TaggingReportDocument{
		...
		URL:          fmt.Sprintf(urlFormat, regionForURL, source.Function.Name),
