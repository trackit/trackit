...
func (as *AnomalySnoozing) Insert(db DB) error {
	...
	const sqlstr = `INSERT INTO trackit.anomaly_snoozing (` +
		`created, user_id, anomaly_id` +
		`) VALUES (` +
		`?, ?, ?` +
		`)`
	// run
	logf(sqlstr, as.Created, as.UserID, as.AnomalyID)
	res, err := db.Exec(sqlstr, as.Created, as.UserID, as.AnomalyID)
	if err != nil {
		return logerror(err)
	}
	// retrieve id
...
func (as *AnomalySnoozing) Update(db DB) error {
	...
	const sqlstr = `UPDATE trackit.anomaly_snoozing SET ` +
		...
		`WHERE id = ?`
	// run
	logf(sqlstr, as.Created, as.UserID, as.AnomalyID, as.ID)
	if _, err := db.Exec(sqlstr, as.Created, as.UserID, as.AnomalyID, as.ID); err != nil {
		return logerror(err)
	}
	return nil
}
...
func (as *AnomalySnoozing) Upsert(db DB) error {
	...
	// upsert
	const sqlstr = `INSERT INTO trackit.anomaly_snoozing (` +
		`id, created, user_id, anomaly_id` +
		`) VALUES (` +
		`?, ?, ?, ?` +
		`)` +
		` ON DUPLICATE KEY UPDATE ` +
		`created = VALUES(created), user_id = VALUES(user_id), anomaly_id = VALUES(anomaly_id)`
	// run
	logf(sqlstr, as.ID, as.Created, as.UserID, as.AnomalyID)
	if _, err := db.Exec(sqlstr, as.ID, as.Created, as.UserID, as.AnomalyID); err != nil {
		return logerror(err)
	}
	// set exists
	as._exists = true
...
func AnomalySnoozingByID(db DB, id int) (*AnomalySnoozing, error) {
	...
	if err := db.QueryRow(sqlstr, id).Scan(&as.ID, &as.Created, &as.UserID, &as.AnomalyID); err != nil {
...
func AnomalySnoozingByUserIDAnomalyID(db DB, userID int, anomalyID string) (*AnomalySnoozing, error) {
	...
	if err := db.QueryRow(sqlstr, userID, anomalyID).Scan(&as.ID, &as.Created, &as.UserID, &as.AnomalyID); err != nil {
		return nil, logerror(err)
	}
	return &as, nil
}
