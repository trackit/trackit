...
func (tdc *TagbotDiscountCode) Insert(db DB) error {
	...
	const sqlstr = `INSERT INTO trackit.tagbot_discount_code (` +
		...
		`)`
	// run
	logf(sqlstr, tdc.Code, tdc.Description, tdc.Created)
	res, err := db.Exec(sqlstr, tdc.Code, tdc.Description, tdc.Created)
	if err != nil {
		return logerror(err)
	}
	// retrieve id
...
func (tdc *TagbotDiscountCode) Update(db DB) error {
	...
	const sqlstr = `UPDATE trackit.tagbot_discount_code SET ` +
		...
		`WHERE id = ?`
	// run
	logf(sqlstr, tdc.Code, tdc.Description, tdc.Created, tdc.ID)
	if _, err := db.Exec(sqlstr, tdc.Code, tdc.Description, tdc.Created, tdc.ID); err != nil {
		return logerror(err)
	}
	return nil
}
...
func (tdc *TagbotDiscountCode) Upsert(db DB) error {
	...
	// upsert
	const sqlstr = `INSERT INTO trackit.tagbot_discount_code (` +
		`id, code, description, created` +
		`) VALUES (` +
		`?, ?, ?, ?` +
		`)` +
		` ON DUPLICATE KEY UPDATE ` +
		`code = VALUES(code), description = VALUES(description), created = VALUES(created)`
	// run
	logf(sqlstr, tdc.ID, tdc.Code, tdc.Description, tdc.Created)
	if _, err := db.Exec(sqlstr, tdc.ID, tdc.Code, tdc.Description, tdc.Created); err != nil {
		return logerror(err)
	}
	// set exists
	tdc._exists = true
...
func TagbotDiscountCodeByID(db DB, id int) (*TagbotDiscountCode, error) {
	...
	if err := db.QueryRow(sqlstr, id).Scan(&tdc.ID, &tdc.Code, &tdc.Description, &tdc.Created); err != nil {
...
func TagbotDiscountCodeByCode(db DB, code string) (*TagbotDiscountCode, error) {
	...
	if err := db.QueryRow(sqlstr, code).Scan(&tdc.ID, &tdc.Code, &tdc.Description, &tdc.Created); err != nil {
		return nil, logerror(err)
	}
	return &tdc, nil
}
