...
func (fp *ForgottenPassword) Update(db DB) error {
	...
	const sqlstr = `UPDATE trackit.forgotten_password SET ` +
		...
		`WHERE id = ?`
	// run
	logf(sqlstr, fp.Created, fp.UserID, fp.Token, fp.ID)
	if _, err := db.Exec(sqlstr, fp.Created, fp.UserID, fp.Token, fp.ID); err != nil {
		return logerror(err)
	}
	return nil
}
...
func (fp *ForgottenPassword) Upsert(db DB) error {
	...
	const sqlstr = `INSERT INTO trackit.forgotten_password (` +
		...
		`created = VALUES(created), user_id = VALUES(user_id), token = VALUES(token)`
	// run
...
func ForgottenPasswordByUserID(db DB, userID int) ([]*ForgottenPassword, error) {
	...
	for rows.Next() {
		...
		// scan
		if err := rows.Scan(&fp.ID, &fp.Created, &fp.UserID, &fp.Token); err != nil {
			return nil, logerror(err)
		}
		res = append(res, &fp)
	}
...
func ForgottenPasswordByID(db DB, id int) (*ForgottenPassword, error) {
	...
	if err := db.QueryRow(sqlstr, id).Scan(&fp.ID, &fp.Created, &fp.UserID, &fp.Token); err != nil {
		return nil, logerror(err)
	}
	return &fp, nil
}
