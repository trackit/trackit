...
func SharedAccountsWithRoleByUserID(db XODB, userID int) ([]*SharedAccountWithRole, error) {
	var err error
	const sqlstr = `SELECT ` +
		`sa.id, sa.account_id, sa.owner_id, sa.user_id, sa.user_permission, sa.account_status, aa.role_arn ` +
	...
	for q.Next() {
		...
		err = q.Scan(&sa.ID, &sa.AccountID, &sa.OwnerID, &sa.UserID, &sa.UserPermission, &sa.AccountStatus, &sa.RoleArn)
		if err != nil {
			return nil, err
		}
		res = append(res, &sa)
