...
type (
	// InstanceReport is saved in ES to have all the information of an EC2SizeRecommendation instance
	InstanceReport struct {
		utils.ReportBase
		Instance Instance `json:"instance"`
	}

	// Instance contains all the information of an EC2SizeRecommendation instance
	Instance struct {
		Id          string `json:"id"`
...
// importInstancesToEs imports EC2SizeRecommendation instances in ElasticSearch.
// It calls createIndexEs if the index doesn't exist.
func importInstancesToEs(ctx context.Context, aa taws.AwsAccount, instances []InstanceReport) error {
	logger := jsonlog.LoggerFromContextOrDefault(ctx)
	logger.Info("Updating EC2SizeRecommendation for AWS account.", map[string]interface{}{
		"awsAccount": aa,
	})
	index := es.IndexNameForUserId(aa.UserId, IndexPrefixEC2SizeRecommendationReport)
	...
	for _, instance := range instances {
		...
		bp = utils.AddDocToBulkProcessor(bp, instance, TypeEC2SizeRecommendationReport, index, id)
	}
	bp.Flush()
	err = bp.Close()
	if err != nil {
		logger.Error("Fail to put EC2SizeRecommendation instances in ES", err.Error())
		return err
	}
	logger.Info("EC2SizeRecommendation instances put in ES", nil)
	return nil
}
