...
describe("Login Saga", () => {
  ...
  it("handles saga with valid data", () => {
    ...
    let saga = loginSaga(credentialsWithoutAWSToken);

    expect(saga.next().value)
      .toEqual(put({ type: Constants.LOGIN_REQUEST_LOADING }));

    expect(saga.next().value)
      .toEqual(call(API.Auth.login, credentials.username, credentials.password, credentialsWithoutAWSToken.awsToken));
  ...
  it("handles saga with valid data & AWS token", () => {
    ...
    let saga = loginSaga(credentialsWithAWSToken);

    expect(saga.next().value)
      .toEqual(put({ type: Constants.LOGIN_REQUEST_LOADING }));

    expect(saga.next().value)
      .toEqual(call(API.Auth.login, credentials.username, credentials.password, credentialsWithAWSToken.awsToken));
