...
export function* getAwsPricingSaga() {
  try {
    ...
    const pricing = yield call(API.getPricing, 'aws', params.frequent, params.infrequent, params.archive );
    yield all([
      put({ type: Constants.AWS_GET_PRICING_SUCCESS, pricing }),
    ]);
  } catch (error) {
    yield put({ type: Constants.AWS_GET_PRICING_ERROR, error });
  }
}
